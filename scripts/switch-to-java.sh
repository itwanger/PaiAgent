...
# 停止 Python 版本
echo -e "${YELLOW}[1/3] 停止 Python Workflow...${NC}"
docker compose stop core-workflow-python 2>/dev/null || true
echo -e "${GREEN}✓ Python Workflow 已停止${NC}"
echo ""

# 启动 Java 版本
echo -e "${YELLOW}[2/3] 启动 Java Workflow...${NC}"
docker compose up -d core-workflow-java
echo -e "${GREEN}✓ Java Workflow 已启动${NC}"
echo ""
